...
const VendasAndamento = () => {
  ...
  const nextAction = (status: string, lanceId: string, lance: any) => {
    switch (status) {
      case "aceito":
        return <Button size="sm" onClick={() => updateStatus(lanceId, "aguardando_pagamento")}>Aguardar Pagamento</Button>;
      ...
      case "pago":
        return <Button size="sm" onClick={() => updateStatus(lanceId, "enviado")}><TruckIcon className="w-3.5 h-3.5 mr-1" /> Confirmar Envio</Button>;
      case "enviado":
        return <Badge variant="outline" className="text-xs">Aguardando recebimento</Badge>;
      default:
        return null;
    }
  };
  ...
  return (
    <DashboardLayout>
      <h1 className="font-display text-2xl font-bold mb-6">Vendas em Andamento</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {lances.map((l) => {
            ...
            return (
              <div key={l.id} className="glass rounded-xl p-4 flex items-center justify-between gap-4 flex-wrap">
